...
sg.theme('Darkred1')
start_layout = [
    [sg.Input(key='-INPUT-'), sg.Button('shkarko')],
]

info_tab = [
    [sg.Text('Titulli:'), sg.Text('', key='-TITLE-')],
    [sg.Text('Gjatesia:'), sg.Text('', key='-LENGTH-')],
    [sg.Text('Shikimet:'), sg.Text('', key='-VIEWS-')],
    [sg.Text('Autori:'), sg.Text('', key='-AUTHOR-')],
    [sg.Text('Pershkrimi:'), sg.Multiline('', key='-DESCRIPTION-', size=(40, 20), no_scrollbar=True, disabled=True)]
]
...
while True:
    ...
    if event == 'submit':
        ...
        # ndertimi i dritares se shkarkimit
